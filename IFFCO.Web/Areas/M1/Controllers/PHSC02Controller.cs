    ...
    public class PHSC02Controller : BaseController<PHSC02ViewModel>
    {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                List<CommonData> data = TechnicalCommonService.GetRecordsPHSC02(controller, "G", EMP_ID.ToString(), DateTime.Now);
            ...
            try
            {
                switch (OperationType)
                {
                    ...
                    case "save":
                        break;
                    case "approve":
                        TechnicalCommonService.ApproveRecordsPHSC02(controller, Shift, EMP_ID.ToString(), FromDate);
                        List<CommonData> data1 = TechnicalCommonService.GetRecordsPHSC02(controller, Shift, EMP_ID.ToString(), FromDate);
                        ViewBag.reason = TechnicalCommonService.GetReason();
        ...
        public IActionResult PostData(string OperationType, string Shift, DateTime FromDate, string Input_Name, string Input_Value, string InputType)
        {
            try
            {
                ...
                string alert = TechnicalCommonService.PostRecordsPHSC02(controller, Shift, EMP_ID.ToString(), FromDate, Input_Value, Input_Name, OperationType);
