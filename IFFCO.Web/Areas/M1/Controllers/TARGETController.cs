    ...
    public class TARGETController : BaseController<TARGETViewModel>
    {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                List<CommonData> data = TechnicalCommonService.GetRecordsTARGET(controller, "G", EMP_ID.ToString(), DateTime.Now.AddDays(-1), DateTime.Now);
        ...
        public IActionResult Execute(string OperationType, string Shift, DateTime FromDate, DateTime ToDate)
        {
            ...
            try
            {
                ...
                {
                    case "query":
                        List<CommonData> data = TechnicalCommonService.GetRecordsTARGET(controller, Shift, EMP_ID.ToString(), FromDate, ToDate);                       
                        ViewBag.records = data;
                    ...
                    case "save":
                        ...
                        return Json(CommonViewModel);
                    case "approve":
                        CommonViewModel.alert = TechnicalCommonService.ApproveRecordsTARGET(FromDate, ToDate);
                        return Json(CommonViewModel);
        ...
        public IActionResult PostData(string OperationType, string Shift, DateTime FromDate, DateTime ToDate, string Input_Name, string Input_Value, string InputType)
        {
            try
            {
                ...
                CommonViewModel.alert = TechnicalCommonService.PostRecordsTARGET(EMP_ID.ToString(), FromDate, ToDate, Input_Value, Input_Name, OperationType);
                return Json(CommonViewModel);
            }
            catch (Exception ex)
            {
